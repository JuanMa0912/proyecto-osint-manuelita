...
En la práctica, OpenFang **auto-extrae** el gateway a `~/.openfang/whatsapp-gateway` y lo lanza junto con el daemon cuando el bloque `[channels.whatsapp]` está en `config.toml` y hay Node ≥ 18. El script `openfang/scripts/06-whatsapp-gateway.sh` deja el gateway **funcional** aplicando los arreglos verificados (§ 7.3): deps con `--omit=optional`, **baileys 7.0.0-rc13** y el parche de direccionamiento LID. La única acción manual ineludible es **escanear el QR**, que vincula el dispositivo. **Verificado en vivo:** tras escanear, un mensaje entrante se procesa y el bot **responde de verdad** (`[gateway] Incoming … → Replied`). Nota de arquitectura WhatsApp Web: el QR vincula el gateway a la cuenta del usuario (es un *dispositivo vinculado* más, como WhatsApp Web), por lo que el "bot" vive en ese número y la prueba de fuego equivale a que **otra persona** escriba a ese número.
...
**Handshake de baileys 6.x incompatible con el protocolo actual (resuelto subiendo a 7.x).** Con la versión que trae el gateway por defecto (`@whiskeysockets/baileys` 6.7.23, el tope de `^6`), el socket conectaba pero la inicialización de la sesión (`executeInitQueries → fetchProps`) **expiraba con timeout 408** de forma sistemática; sin completar ese *handshake*, baileys no descifraba los mensajes entrantes (`Invalid PreKey ID`) ni podía responder. Se descartó que fuera de red: desde WSL, `web.whatsapp.com` y el *endpoint* WebSocket respondían en < 1 s. La causa era la **versión de la librería** frente al protocolo vigente de WhatsApp (jun 2026). **Fix:** fijar `@whiskeysockets/baileys` a **`7.0.0-rc13`** (`added 98 packages`, *import* validado); con 7.x el *init* completa (0 *timeouts*) y el descifrado funciona. Caveat: las credenciales de un emparejamiento 6.x **no** sirven en 7.x → re-escanear una vez con la sesión limpia.

**Instalación del gateway frágil (resuelto con `--omit=optional` y un solo `npm install`).** El `npm install` automático del gateway se colgaba por dos motivos: (1) **doble** `npm install` simultáneo —el que lanza el daemon más cualquier intento manual— se pisaban sobre el mismo `node_modules` y dejaban `@whiskeysockets/baileys` a medias (`Cannot find package baileys`); y (2) algunas **dependencias opcionales nativas** intentaban compilar sin *toolchain* (`make`/`gcc` ausentes) y quedaban colgadas. **Fix:** con el daemon apagado, un único `npm install --omit=optional`, que completa en ~10 s.

**La respuesta no se entregaba por direccionamiento LID (resuelto con un parche en `index.js`).** WhatsApp introdujo el direccionamiento **LID** (*Linked ID*): los mensajes entrantes llegan con `remoteJid = <lid>@lid` y el número real en `remoteJidAlt`. El gateway construía el destinatario de la respuesta como `<lid>@s.whatsapp.net` —un número inexistente—, por lo que registraba `Replied` pero el mensaje **no se entregaba**. **Fix:** un parche en `index.js` (línea del `replyJid`) que, cuando el mensaje viene en `@lid`, responde a `msg.key.remoteJidAlt` (el número real). Tras el parche, el bot responde y el mensaje llega. Este parche y los dos anteriores quedan automatizados en `scripts/06-whatsapp-gateway.sh` (el gateway lo auto-extrae el binario y no se versiona).
...
- **Pruebas en vivo: ensayo hecho, sustentación pendiente.** El flujo de Telegram se **ensayó en vivo desde un teléfono real** (prueba de fuego validada); resta la ejecución el día de la sustentación con el teléfono del evaluador. **WhatsApp quedó verificado respondiendo** tras resolver los tres obstáculos del puente WhatsApp Web (baileys 7, instalación, LID, § 7.3); como es un puente no oficial, conviene reconectar/re-escanear con tiempo el día de la demo y, por su naturaleza multi-dispositivo, probarlo con otro teléfono escribiendo al número vinculado.
