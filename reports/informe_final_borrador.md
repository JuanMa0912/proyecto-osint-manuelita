...
Manuelita S.A. es la organización corporativa sobre la que se construye este sistema. El proyecto la trata como un grupo empresarial con presencia internacional (opera en varios países [POR VERIFICAR: el listado exacto de países está en `data/structured/manuelita_datos.json`, no en los archivos leídos]) y con una estructura de negocio organizada en **siete unidades de negocio** y **cuatro plataformas principales**, según la estructura del archivo de datos estructurados descrita en MODULO2.md. La información de la empresa que el sistema maneja se divide en dos grandes naturalezas:

- **Datos exactos y verificables:** NIT, ciudad y año de fundación, presidente, países y sedes, número de colaboradores/proveedores/clientes, histórico financiero (ingresos, EBITDA y utilidad neta para el periodo 2019–2024), metas de carbono, certificaciones y entidades relacionadas del grupo. Estos datos residen en `data/structured/manuelita_datos.json` [POR VERIFICAR: los valores numéricos concretos no aparecen en CLAUDE.md ni MODULO2.md].
...
El sistema final del Módulo 3 se concibe como un **agente conversacional corporativo de Manuelita desplegado sobre OpenFang**, un Agent OS open source escrito en Rust (versión fijada **v0.6.9, pre-1.0**, según se verifica en CLAUDE.md), ejecutado sobre **WSL2 (Ubuntu) en Windows 11**. Sus elementos previstos son:
...
OpenFang es un Agent OS open source escrito en **Rust**, con licencia MIT [POR VERIFICAR: licencia MIT y carácter open source provienen de CLAUDE.md, no del README de la carpeta `openfang/`]. Se ejecuta en **WSL2 (Ubuntu)** sobre Windows 11, con su dashboard local en `http://127.0.0.1:4200`.
...
| Routing | — | Router híbrido por keywords | Razonamiento del agente OpenFang [POR VERIFICAR] |
...
El binario se instala vía `curl -fsSL https://openfang.sh/install | sh`, quedando en `/root/.openfang/bin/openfang`. La configuración del modelo vive en `~/.openfang/config.toml` (sección `[default_model]`), lo que permite **intercambiar el proveedor LLM** entre un modo demo (Gemini) y un modo de soberanía de datos (Ollama local, endpoint `http://localhost:11434/v1`).
...
A partir de lo verificado en los archivos del repositorio, las ventajas operativas constatadas de adoptar OpenFang como Agent OS son:
...
Respecto a las ventajas que el enunciado del Módulo 3 lista como puntos a tratar en el informe final —**seguridad WASM** y **gestión de RAM** del Agent OS—, **no están respaldadas por los archivos leídos**: el documento de spike F0 (`openfang/docs/F0-spike-infraestructura.md`) no menciona ni aislamiento/sandbox WASM ni administración de memoria RAM, y en `CLAUDE.md` aparecen únicamente como temas a cubrir en el entregable, no como hechos verificados.

- Seguridad WASM (aislamiento/sandbox de los Hands): **[POR VERIFICAR]** — no documentada en los archivos del repo indicados.
- Gestión de RAM del Agent OS: **[POR VERIFICAR]** — no documentada en los archivos del repo indicados.
...
- 4 plataformas: azúcar, palma de aceite, acuicultura, y frutas y hortalizas.
...
- Cifras operativas (perfil corporativo): ~487.000 toneladas/año de azúcar; ~275 millones de litros de bioetanol; exporta a ~65 países; más de 160 años de trayectoria.
...
> Estos valores provienen del `system_prompt` del agente `manuelita-bot` documentado en el repo; su contraste literal contra `data/structured/manuelita_datos.json` queda [POR VERIFICAR] (ver Pendientes).
...
**Decisión de modelo:** el agente usa `gemini-2.5-flash` (no el lite), fijado como override por-agente en `agent.toml` (`[model] provider = "gemini"`, `model = "gemini-2.5-flash"`), con **fallback** a `gemini-2.5-flash-lite` declarado en `[[fallback_models]]` por si topa cuota. El comentario del propio `agent.toml` documenta esta decisión y su costo (~20 s y ~10× tokens en preguntas profundas frente al lite). [POR VERIFICAR] La sección 6 del documento F1 todavía describe `provider = "default"` heredando `gemini-2.5-flash-lite` como configuración inherente, lo que contradice el override por-agente del `agent.toml`; el `agent.toml` versionado se asume como la fuente de verdad operativa.
...
El modelo de memoria de OpenFang se organiza en capas. Con respaldo directo en los archivos del repo:

| Capa | Naturaleza | Soporte en el repo |
|------|-----------|--------------------|
| **Sesiones** | Historial de las conversaciones, extraíble para análisis posterior (p. ej. el t-SNE opcional de la fase F5) | CLAUDE.md las describe como "JSONL/SQLite FTS5" [POR VERIFICAR el formato exacto y la ruta] |
| **`MEMORY.md`** | *Long-Term Memory* curada: conocimiento clave a través de sesiones | Verificado: se curó el `MEMORY.md` del workspace de `manuelita-bot` |
| **KV (clave-valor)** | Datos estructurados accesibles vía `memory_store` / `memory_recall` | Verificado: `memory` es solo KV (list/get/set/delete); persiste en SQLite [POR VERIFICAR que la persistencia KV use exactamente `openfang.db`] |
| **`memory/<fecha>.md`** | Memoria diaria por fecha | [POR VERIFICAR] — ninguno de los archivos leídos menciona una capa `memory/<fecha>.md` |

La capa de sesiones se materializa, en el daemon, junto con el resto del estado en la base de datos `openfang.db*`; tras desplegar un agente nuevo es necesario **borrar `openfang.db*` y reiniciar** para que OpenFang cargue el manifiesto actualizado (de lo contrario el daemon revive los 30 templates registrados en la DB, no solo en disco).
...
> **Nota sobre el conteo de Hands built-in:** el documento F2 y el `HAND.toml` indican **9 built-in**, mientras que `CLAUDE.md` menciona **7 Hands incluidos**. [POR VERIFICAR] esta discrepancia.
...
[POR VERIFICAR] La configuración quota-safe equivalente del `lead` (built-in) y el `system_prompt`/comportamiento interno del `collector` no constan en los archivos leídos (solo su configuración).
...
1. **Token en el entorno del daemon.** El token del bot (obtenido de BotFather [POR VERIFICAR: la fuente F3 no menciona explícitamente BotFang/BotFather; CLAUDE.md sí]) se coloca en `~/.openfang/manuelita.env` —que no se commitea— y se protege con `chmod 600`.
...
**Conflicto 409 por doble daemon.** El enunciado pide cubrir un conflicto HTTP 409 por doble daemon; sin embargo, el documento de la fase F3 (`openfang/docs/F3-canales.md`) **no describe ningún error 409 ni un escenario de doble daemon** [POR VERIFICAR]. Los errores HTTP documentados en esa fuente son el `400` ("Invalid agent ID") y el `404` (al usar `channel enable` sobre un canal sin configurar). Este punto debe verificarse en otra fuente del repositorio antes de incluirlo en el informe final.
...
- **Fragilidad pre-1.0.** La versión v0.6.9 obligó a numerosos *workarounds* (agentes zombie en la DB, `base_url` de Ollama con `/v1`, UUID v4 cambiante, binding inconsistente nombre/UUID entre Telegram y REST), y varias afirmaciones del enunciado no se pudieron sustanciar en los archivos del repo (seguridad WASM, gestión de RAM, conflicto 409 por doble daemon).
...
- **Datos corporativos sin contrastar contra el JSON.** Los valores numéricos del agente (NIT, cifras, presidente, etc.) provienen de su `system_prompt`/corpus; su contraste literal contra `data/structured/manuelita_datos.json` no se realizó (ver § 10).

---

## 10. Pendientes por verificar

1. **Valores numéricos concretos de Manuelita** (NIT, ingresos, EBITDA, utilidad neta 2019–2024, número de colaboradores, año de fundación, metas de carbono): residen en `data/structured/manuelita_datos.json`, pero NO aparecen en CLAUDE.md ni MODULO2.md, los únicos archivos leídos. POR VERIFICAR contra el JSON.
2. **Listado exacto de países y sedes** en que opera Manuelita: referenciado como categoría 'paises'/'geografia' pero sin valores explícitos en los archivos leídos. POR VERIFICAR contra el JSON.
3. **Nombre del presidente / directivos**: categoría existente en el JSON, sin valor literal en CLAUDE.md ni MODULO2.md. POR VERIFICAR contra el JSON.
4. **Detalle de las 7 unidades de negocio y las 4 plataformas principales**: mencionadas como estructura del JSON, sin enumeración concreta en los archivos leídos. POR VERIFICAR contra el JSON.
5. **Naturaleza de Manuelita como "grupo empresarial con presencia internacional"** es una inferencia a partir de la existencia de `filiales_relacionadas`, `geografia` y unidades de negocio; no hay una descripción corporativa textual de la empresa en CLAUDE.md ni MODULO2.md.
6. **Licencia MIT y carácter "open source" de OpenFang**: figuran en CLAUDE.md pero NO en los archivos del repo indicados como fuente primaria (`openfang/README.md` no lo menciona). Marcado [POR VERIFICAR].
7. **Versión de OpenFang (v0.6.9)**: aparece en CLAUDE.md pero el README de la carpeta `openfang/` no la cita.
8. **Mecanismo de "routing" en M3 con OpenFang**: en M2 el router híbrido por keywords está documentado; en M3, cómo decide el agente OpenFang qué herramienta/archivo usar no está detallado en los archivos leídos. Marcado [POR VERIFICAR] en la tabla de evolución.
9. **Cifras de calidad RAG del M2** (~86% local, ~95% gemini estimado, ~60% ollama) provienen de MODULO2.md como resultados/estimaciones; el ~95% de gemini está explícitamente marcado como "estimado" en la fuente.
10. **Detalles de OpenFang citados solo en CLAUDE.md** (binario ~32MB, 7 Hands, 27 proveedores LLM, ~40 adaptadores de canal): NO incluidos en el cuerpo por no estar respaldados en MODULO2.md ni en `openfang/README.md` ni en el F0 spike.
11. **Seguridad WASM** (aislamiento/sandbox de los Hands): mencionada en CLAUDE.md solo como tema del entregable, NO substanciada en `F0-spike-infraestructura.md` ni con detalle técnico verificable. [POR VERIFICAR]
12. **Gestión de RAM del Agent OS**: listada en CLAUDE.md como ventaja a tratar, pero sin ningún dato técnico de respaldo en los archivos leídos. [POR VERIFICAR]
13. **Contraste funcional Ruta A** (FastAPI+Function Calling+N8N): los archivos confirman que existía como alternativa y que se descartó, pero no describen sus ventajas/desventajas técnicas; el contraste se limita a la consecuencia arquitectónica documentada (reuso del corpus M1, no del código M2).
14. **Contradicción de proveedor del agente**: la sección 6 de `F1-agente-manuelita.md` indica `provider = "default"` heredando `gemini-2.5-flash-lite`, mientras que `agent.toml` fija `model = "gemini-2.5-flash"` como override por-agente; ambos archivos se contradicen y no es posible determinar cuál refleja el estado desplegado final (se asume `agent.toml` como fuente de verdad operativa).
15. **Conteo de tokens y latencias del spike** (~7.500 / ~24.000 tokens; 3–9 s / 7–20 s) provienen de la tabla del spike descrita como "aprox."; no hay logs ni evidencia bruta en los archivos leídos que respalde las mediciones exactas.
16. **Capa `memory/<fecha>.md`** (memoria diaria por fecha): NO aparece en `F1-agente-manuelita.md` ni en CLAUDE.md.
17. **Formato/ruta exacto de la capa de sesiones**: descrito en CLAUDE.md solo como "JSONL/SQLite FTS5" en el contexto del t-SNE opcional (F5); no se confirma ruta de archivo ni esquema concreto.
18. **Persistencia KV en `openfang.db`**: CLAUDE.md menciona "borrar `openfang.db*`" para los templates y que `memory` es KV en SQLite, pero no liga explícitamente el KV a ese archivo.
19. **Tope de cuota `max_llm_tokens_per_hour = 200000`** (~8 preguntas profundas/hora): proviene de `F1-agente-manuelita.md`; se cita como dato del repo, no verificado contra OpenFang.
20. **Conteo de Hands built-in**: `F2-hands.md` y el `HAND.toml` indican "9 built-in", mientras que CLAUDE.md menciona "7 Hands incluidos". Discrepancia [POR VERIFICAR].
21. **Configuración quota-safe del `lead` (built-in)**: no aparece detallada en los archivos leídos; solo se documenta la del `collector` y la de la Hand custom. [POR VERIFICAR]
22. **`system_prompt` y comportamiento interno del `collector` (built-in)**: no constan en los archivos leídos (solo su configuración). [POR VERIFICAR]
23. **Conflicto HTTP 409 por doble daemon** que pide el enunciado: NO aparece en `openfang/docs/F3-canales.md`; la fuente solo documenta errores 400 (Invalid agent ID) y 404 (channel enable sobre canal sin configurar). Verificar en otra fuente del repo.
24. **Origen BotFather del token de Telegram**: el documento F3 no lo menciona explícitamente (CLAUDE.md sí lo indica, fuera del alcance de esa fuente). Marcado inline como [POR VERIFICAR].